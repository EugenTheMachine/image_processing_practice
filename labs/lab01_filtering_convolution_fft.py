...
def conv2d(image: npt.NDArray[np.generic], kernel: npt.NDArray[np.generic], border: BorderType = "reflect") -> np.ndarray:
    ...
    boundary_map = {"reflect": "symm", "constant": "fill", "wrap": "wrap"}

    img = np.asarray(image, dtype=np.float32)
    if img.ndim == 2:
        if border in boundary_map:
            return signal.convolve2d(img, k, mode="same", boundary=boundary_map[border], fillvalue=0.0).astype(
                np.float32, copy=False
            )
        if border == "replicate":
            pad_y, pad_x = k.shape[0] // 2, k.shape[1] // 2
            padded = np.pad(img, ((pad_y, pad_y), (pad_x, pad_x)), mode="edge")
            return signal.convolve2d(padded, k, mode="valid").astype(np.float32, copy=False)
        raise ValueError("Unsupported border")

    channels = []
    for c in range(img.shape[2]):
        channels.append(conv2d(img[..., c], k, border=border))
...
def make_gaussian_kernel(ksize: int, sigma: float) -> npt.NDArray[np.float32]:
    ...
    g1 = cv2.getGaussianKernel(ksize, sigma).astype(np.float32, copy=False)
    k = (g1 @ g1.T).astype(np.float32, copy=False)
    k /= float(np.sum(k))
    return k
...
def apply_gaussian_blur(image: npt.NDArray[np.generic], ksize: int, sigma: float) -> np.ndarray:
    ...
    y = conv2d(image, k, border="reflect")
    return _clip_to_dtype_range(y, np.asarray(image).dtype).astype(np.asarray(image).dtype, copy=False)
...
def apply_median_blur(image: npt.NDArray[np.generic], ksize: int) -> np.ndarray:
    ...
    if img.dtype != np.uint8:
        raise TypeError("apply_median_blur currently supports uint8 images")
...
def add_salt_pepper_noise(
    ...
    *,
    ...
    n_total = out.size
    n_corrupt = int(round(amount * n_total))
    if n_corrupt == 0:
        return out

    idx = rng.integers(0, n_total, size=n_corrupt, endpoint=False)
    salt_mask = rng.random(size=n_corrupt) < salt_vs_pepper

    if np.issubdtype(out.dtype, np.integer):
        lo, hi = np.iinfo(out.dtype).min, np.iinfo(out.dtype).max
    else:
        lo, hi = 0.0, 1.0

    flat = out.reshape(-1)
    flat[idx[salt_mask]] = hi
    flat[idx[~salt_mask]] = lo
    return out
...
def add_gaussian_noise(image: npt.NDArray[np.generic], sigma: float, *, seed: int = 0) -> np.ndarray:
    ...
    y = img.astype(np.float32) + noise
    y = _clip_to_dtype_range(y, img.dtype)
...
def sobel_edges(image: npt.NDArray[np.generic], ksize: int = 3) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if img.ndim == 3:
        ...
        img_u8 = img_u8[..., :3]
        img = cv2.cvtColor(img_u8, cv2.COLOR_BGR2GRAY)
    else:
        img = img if img.dtype == np.uint8 else img.astype(np.uint8)
    img_f = img.astype(np.float32)
    gx = cv2.Sobel(img_f, cv2.CV_32F, 1, 0, ksize=ksize)
    gy = cv2.Sobel(img_f, cv2.CV_32F, 0, 1, ksize=ksize)
    mag = cv2.magnitude(gx, gy)
    return gx.astype(np.float32, copy=False), gy.astype(np.float32, copy=False), mag.astype(np.float32, copy=False)
...
def fft2_image(image: npt.NDArray[np.generic]) -> np.ndarray:
    ...
    return dft.astype(np.float32, copy=False)
...
def fftshift2(spectrum: npt.NDArray[np.floating]) -> np.ndarray:
    ...
    if spec.ndim not in (2, 3):
        raise ValueError("spectrum must be 2D or 3D (2-channel)")
...
def magnitude_spectrum(spectrum: npt.NDArray[np.floating], log_scale: bool = True) -> np.ndarray:
    ...
    if spec.ndim != 3 or spec.shape[2] != 2:
        raise ValueError("Expected OpenCV 2-channel spectrum of shape (H,W,2)")
...
def ideal_low_pass_filter(shape: tuple[int, int] | tuple[int, int, int], cutoff_radius: float) -> np.ndarray:
    ...
    mask2 = (dist <= float(cutoff_radius)).astype(np.float32)
    return np.dstack([mask2, mask2]).astype(np.float32, copy=False)
...
def ideal_high_pass_filter(shape: tuple[int, int] | tuple[int, int, int], cutoff_radius: float) -> np.ndarray:
    ...
    """
    lp = ideal_low_pass_filter(shape, cutoff_radius)
    return (1.0 - lp).astype(np.float32, copy=False)
...
def apply_frequency_filter(image: npt.NDArray[np.generic], filter_mask: npt.NDArray[np.floating]) -> np.ndarray:
    ...
    if mask.shape[:2] != dft_shift.shape[:2] or mask.shape[2] != 2:
        raise ValueError("filter_mask must match spectrum shape (H,W,2) or (H,W)")

    filtered = dft_shift * mask
    filtered_ishift = np.fft.ifftshift(filtered, axes=(0, 1))
    img_back = cv2.idft(filtered_ishift)
    spatial = cv2.magnitude(img_back[..., 0], img_back[..., 1])
    return spatial.astype(np.float32, copy=False)
